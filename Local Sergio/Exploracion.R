...
conciertos <- df_conciertos_final %>%
  mutate(recinto = str_squish(recinto)) %>% 
  left_join(recintos_eq, by = c("recinto" = "raw")) %>%
  mutate(recinto_canonico = coalesce(canonico, recinto)) %>%
  select(-canonico)
...
conciertos_dia <- conciertos %>%
  ...
  summarise(
    ...
    max_aforo     = suppressWarnings(max(aforo, na.rm = TRUE)),
...
ocup_diaria_06 <- df_calendar_06 %>%
  group_by(fecha = date) %>%
  summarise(
    tasa_ocupacion = mean(available == FALSE, na.rm = TRUE),
...
listings_clean <- df_listings_06 %>%
  ...
  rename(barrio   = neighbourhood_cleansed,
         ...
         precio   = price) %>%
  mutate(
    tasa_ocup_proxy = estimated_occupancy_l365d / 365,  # 0-1
...
tabla_mensual <- tabla_diaria %>%
  group_by(anio, mes) %>%
  summarise(
    fecha_mes          = floor_date(min(fecha), "month"),
...
t2 <- listings_clean %>%
  ...
  summarise(
    ...
    precio_p25   = round(quantile(precio, 0.25, na.rm = TRUE), 1),
    precio_p75   = round(quantile(precio, 0.75, na.rm = TRUE), 1),
    .groups = "drop"
  ) %>%
  arrange(desc(n))
print(t2)
...
t4 <- tabla_mensual %>%
  ...
  mutate(
    cuartil = ntile(n_conciertos_mes, 4),
...
fig1 <- tabla_mensual %>%
  ...
  geom_smooth(method = "loess", se = FALSE, color = C_EVENTO, linewidth = 1) +
...
fig3 <- conciertos %>%
  ...
  slice_max(n, n = 20) %>%
  mutate(recinto_canonico = fct_reorder(recinto_canonico, n)) %>%
...
# Test preliminar H1 (ocupación, no precio)
...
fig_ocup_conc <- tabla_diaria_airbnb %>%
  ...
  geom_point(aes(color = tipo_dia), alpha = 0.4, size = 1.2) +
...
# Correlación de Spearman (incluir en texto)
cor_test <- cor.test(
...
# NUEVA REPRESENTACIÓN DE CORRELACIONES
# Paso 1: matriz con p-values (paquete Hmisc)
...
# (Opcional pero recomendado) Aplicar los nombres limpios a las nuevas matrices
nombres_limpios <- c(
  "Conciertos/mes", "Grandes eventos", "Max. listeners",
  "Ocup. hotelera", "Viajeros total", "Viajeros nac.",
  "Viajeros int.", "Pernoctaciones", "IPH", "Ocup. aptos."
...
# 2. Creamos la matriz de colores para el texto
color_texto <- matrix("black", nrow = nrow(cor_r), ncol = ncol(cor_r))
color_texto[cor_p > 0.05] <- "grey60"  # Gris para las no significativas
...
# Pintamos el gráfico SIN pasarle p.mat para evitar el colapso de filas/columnas
corrplot(cor_r,
         method      = "color",
         type        = "upper",
         tl.cex      = 0.72,
...
# Coordenadas CORREGIDAS de los principales recintos (WGS84)
...
# Nº de conciertos por venue (para el tamaño de los puntos)
...
precio_barrio <- listings_clean %>%
  ...
  summarise(precio_mediano = median(precio, na.rm = TRUE),
            n_listings     = n(),
            .groups = "drop") %>%
  # SILENCIAMOS LOS BARRIOS CON POCOS LISTINGS (< 50)
  mutate(precio_mediano = if_else(n_listings >= 50, precio_mediano, NA_real_))

# Unir precio al geojson de barrios
