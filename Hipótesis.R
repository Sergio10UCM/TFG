...
# Vector con librerías avanzadas para modelado econométrico, espacial y reporte
paquetes_cap4 <- c(
  "tidyverse", "lubridate",
  "fixest",         # Modelos de panel con múltiples efectos fijos de alta dimensionalidad
  "modelsummary",   # Generación de tablas comparativas de modelos en formato APA
  "broom",          # Extracción 'tidy' de resultados de modelos
  "forecast",       # Modelado de series temporales (ARIMA, descomposición STL)
  "tseries",        # Tests estadísticos para series temporales (ej. Dickey-Fuller)
  "lmtest",         # Tests de causalidad de Granger y testeo de coeficientes
  "sf",             # Manejo de geometrías espaciales vectoriales
  "spgwr",          # Regresión Geográficamente Ponderada (GWR)
  "geosphere",      # Cálculo de distancias geoespaciales (Haversine)
  "flextable",      # Creación de tablas enriquecidas para documentos Word
  "officer"         # Manipulación de documentos Word desde R
)

# Instalación condicional de paquetes faltantes
...
# Creación del directorio para almacenar los outputs (tablas, figuras y datos exportados)
# showWarnings = FALSE evita mensajes de error si la carpeta ya existe
...
# Transformación de datos transversales a panel longitudinal (Listing-Día)
panel_h1 <- df_calendar_06 %>%
  mutate(fecha = as.Date(date)) %>%
  select(listing_id, fecha, available) %>%
  # Creación de la variable dependiente binaria (1 = Ocupado/Bloqueado)
  mutate(ocupado = if_else(available == FALSE, 1L, 0L)) %>%
  # Incorporación de variables explicativas a nivel temporal (días)
  left_join(
  ...
  # Incorporación de variables de control a nivel espacial/listing
  left_join(
    listings_clean %>% select(id, barrio, distrito, room_type, accommodates),
    by = c("listing_id" = "id")
  ) %>%
  # Limpieza de valores atípicos/incompletos
  filter(!is.na(barrio), !is.na(room_type)) %>%
  mutate(
    listing_id  = as.character(listing_id),
    mes_anio    = factor(paste0(anio, "-", sprintf("%02d", mes))), # Control para estacionalidad mensual
    dia_semana  = factor(dia_semana,
                         levels = c("Lunes","Martes","Miércoles","Jueves",
                                    "Viernes","Sábado","Domingo")),
    # Dummy principal de impacto: 1 si hay al menos un gran evento ese día
...
# Modelo (1) BASELINE — Modelo Lineal de Probabilidad (LPM) con Efectos Fijos
# Absorbe la heterogeneidad no observada invariante en el tiempo por alojamiento y mes
mod_h1_1 <- feols(
  ocupado ~ es_gran_ev + n_conciertos + hay_concierto
  | listing_id + mes_anio,
  data    = panel_h1,
  cluster = ~ listing_id # Errores estándar clusterizados a nivel de alojamiento para autocorrelación
)

# Modelo (2) — Se añaden controles temporales de alta frecuencia (día de la semana y festivos)
# que varían dentro del mismo mes
mod_h1_2 <- feols(
  ocupado ~ es_gran_ev + n_conciertos + hay_concierto
...
# Modelo (3) — Disminución de la granularidad de los Efectos Fijos (Barrio en lugar de Listing)
# Permite evaluar cómo cambia el ajuste al relajar los controles espaciales e incluir tipo de cuarto
mod_h1_3 <- feols(
  ocupado ~ es_gran_ev + n_conciertos + hay_concierto
...
# Modelo (4) POOLED PURO — Regresión OLS estándar sin efectos fijos restrictivos
# Sirve como línea base algorítmica para entender la varianza cruda (R2 total vs R2 within)
mod_h1_4 <- feols(
  ocupado ~ es_gran_ev + n_conciertos + hay_concierto
  + dia_semana + es_festivo + room_type,
  data    = panel_h1,
...
# Partición temporal de la muestra (80% entrenamiento, 20% testeo)
...
# Entrenamiento del modelo principal (2) solo con los datos de train
mod_h1_train <- feols(
  ocupado ~ es_gran_ev + n_conciertos + hay_concierto
  + dia_semana + es_festivo
  | listing_id + mes_anio,
...
# Predicción en el set de testeo: Filtramos listings y meses que existen en el modelo entrenado
# para evitar errores de predicción por falta de niveles en los efectos fijos
...
# Cálculo manual de métricas de rendimiento Out-Of-Sample
...
# Modelo de contraste que evalúa la capacidad base (sin efecto conciertos)
mod_h1_nulo <- feols(
...
# Lista nombrada que será consumida por `modelsummary`
modelos_panel <- list(
  "(1) H1 base"           = mod_h1_1,
...
# Extracción de coeficientes y p-valores para el flujo de texto de la memoria
...
# Modelado logístico para contrastar la estabilidad funcional del Modelo Lineal de Probabilidad
mod_h1_logit <- feglm(
  ocupado ~ es_gran_ev + n_conciertos + hay_concierto
  + dia_semana + es_festivo
  | listing_id + mes_anio,
...
# AME = Efecto Marginal Promedio (Average Marginal Effect)
# Sirve para transformar log-odds de vuelta a probabilidad y poder comparar con el OLS
...
# Test de Wald para medir si los factores de tiempo en bloque son significativos
...
# Transformación de oyentes a log10 para evitar leverage excesivo de mega-artistas
panel_h3 <- panel_h1 %>%
  mutate(log_listeners = log10(replace_na(max_listeners, 1) + 1))

# Modelo (5) — Introducción de interacción multiplicativa
mod_h3_int <- feols(
...
# Descomposición analítica del efecto marginal para calcular la varianza conjunta
...
# Escenarios de estrés para evaluar el efecto en diferentes niveles de mercado
niveles_listeners <- c(1e5, 5e5, 1e6, 3e6, 5e6, 1e7, 5e7)

# Cálculo manual de efectos marginales e Intervalos de Confianza
efecto_marginal <- tibble(
  listeners = niveles_listeners,
  log_list  = log10(niveles_listeners + 1),
  ef_marg   = beta_ev + beta_int * log_list,
  # Fórmula de varianza para combinación lineal de parámetros con covarianza
...
# Umbral empírico donde el efecto global supera el cero: cruzamiento de ejes
...
# Diccionario de nombres legibles para reemplazar variables codificadas en el output APA
coef_labels <- c(
...
# Exportación automatizada a DOCX
modelsummary(
  modelos_panel,
  output      = "cap4_outputs/tabla_maestra_panel.docx",
  coef_map    = coef_labels,
  coef_omit   = "barrio|mes_anio", # Ocultamiento de FEs en la tabla final
...
cat("  Tabla guardada en cap4_outputs/tabla_maestra_panel.html\n\n") # Nota: El archivo se llama docx arriba

# Versión markdown para inspeccionar rápidamente los resultados en la consola interactiva
...
# Diccionario posicional de recintos para cálculos geodésicos
venues_h5 <- tribble(
  ~recinto,                           ~lat,      ~lon,
...
# Extracción de coordenadas únicamente para los recintos con actividad constatada
...
# Función iterativa para calcular la distancia Haversine al venue geográficamente más cercano
...
# Aplicación del cálculo vectorial y estratificación en anillos discretos
...
# CORRECCIÓN: Conversión explícita del 'id' a caracter.
# Previene problemas de desajuste de tipos (integer/numeric vs character) durante el left_join.
panel_h5 <- panel_h1 %>%
...
# Ajuste iterativo del Modelo de Efectos Fijos sub-segmentando por anillo de cercanía
resultados_anillos <- map_dfr(levels(panel_h5$anillo), function(a) {
  df_a <- panel_h5 %>% filter(anillo == a)
  if (nrow(df_a) < 100) return(NULL) # Umbral de tamaño muestral mínimo
  m <- tryCatch(
  ...
  # Estructuración de salida en formato tabla Tidy
  tibble(
    anillo  = a,
    n_obs   = nrow(df_a),
...
# Cálculo de correlación para evaluar la monotonicidad del decaimiento
...
# CORRECCIÓN: Coerción del id a caracter para el left join como se hizo arriba
...
# Proyección cartográfica a UTM (ETRS89 / UTM zone 30N) para calcular distancias
# reales precisas en metros a la hora de estructurar la regresión local.
...
# Búsqueda adaptativa del parámetro de ancho de banda espacial para el GWR
bw <- tryCatch(
  gwr.sel(y ~ dist_km,
          data    = x_df,
          coords  = coords,
...
# Modelo Geographically Weighted Regression final
...
# Exportar los β locales de vuelta al espacio geográfico de Airbnb para visualización externa
...
# Conversión técnica requerida por el paquete GWmodel, usa objetos sp
...
# Estimación del GWLR (Geographically Weighted Logistic Regression)
...
# Test de Dickey-Fuller aumentado para descartar raíces unitarias espurias
...
# Descomposición de series estacionales y de tendencia usando LOESS (STL)
stl_ocup <- stl(ts_ocup, s.window = "periodic", robust = TRUE)
stl_conc <- stl(ts_conc, s.window = "periodic", robust = TRUE)
# Reconstrucción de series desestacionalizadas
...
# Exportación de visualizaciones a PNG
...
# Auto.arima busca el mejor modelo según AICc minimizado, integrando la exógena
...
# Especificación explícita alternativa de la temporalidad para control manual
df_alt <- data.frame(
...
# H6 panel con ocupación Airbnb (para modelos XGBoost en Python)
tabla_diaria_airbnb %>%
...
# H7: dataset de conciertos para clustering de aprendizaje no supervisado
conciertos %>%
...
# Modelo nulo de recapitulación (solo efectos fijos + controles temporales)
mod_h1_nulo <- feols(
  ocupado ~ dia_semana + es_festivo | listing_id + mes_anio,
...
# Carga de extensiones orientadas a maquetación y formateo final
library(tidyverse)
library(flextable)
library(officer)
library(broom)
...
# -----------------------------------------------------------------------------
# Funciones auxiliares de formateo APA
# -----------------------------------------------------------------------------

# Función para dar formato a números españoles (coma flotante, etc)
fmt_es <- function(x, dig = 3) {
  if (length(x) == 0 || is.na(x)) return("")
  formatC(x, format = "f", digits = dig, decimal.mark = ",")
}

# Función para formatear P-Valores estándar a < 0,001 en los bordes
fmt_p <- function(p) {
  if (is.na(p)) return("")
...
# Formateador maestro Flextable a normativa APA 7th Edition
...
# Interfaz automática de guardado a Word
...
# Integración Modelsummary con Flextable vía output = "flextable"
ft_4_1 <- modelsummary(
  modelos_panel,
  output    = "flextable",
  coef_map  = coef_labels,
...
# Procesamiento de la tabla cruda extraída de las regresiones estratificadas
...
# Extracción y embellecimiento de parámetros ARIMA
ct_arimax <- lmtest::coeftest(mod_arimax)
...
# Cuadro resumen con conclusiones de los modelos para memoria final
tabla_4_12 <- tribble(
